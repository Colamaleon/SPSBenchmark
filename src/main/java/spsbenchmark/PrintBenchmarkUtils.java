...
public class PrintBenchmarkUtils {
     ...
     */
    public static void prettyPrintConfig(BenchmarkConfig config, BenchmarkMode mode) {

        System.out.println(separator());

        String startMessage = String.format("Running %s benchmark with config... ",
                (mode == BenchmarkMode.Counting) ? "[Counting]" : "[Timer]");

        System.out.println(padString(startMessage, CONSOLE_WIDTH));

        System.out.println(padString((mode == BenchmarkMode.Counting) ?
